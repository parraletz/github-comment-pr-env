...
__nccwpck_require__.d(__webpack_exports__, {
  checkAndCreateComment: () => (/* binding */ checkAndCreateComment)
});
...
const owner = process.env.REPO_OWNER || process.env.PLUGIN_REPO_OWNER || 'parraletz';
...
const message = process.env.PR_MESSAGE || process.env.PLUGIN_PR_MESSAGE || '';
const githubToken = process.env.GITHUB_TOKEN || process.env.PLUGIN_GITHUB_TOKEN || '';
const commitSha = process.env.COMMIT_SHA || process.env.PLUGIN_COMMIT_SHA || '';
const octokit = new dist_src_Octokit({ auth: githubToken });
const createMessagePreviewEnvironment = (commitSha, repo, url, owner) => {
    const message = `
...
async function checkAndCreateComment(owner, repo, pull_number, message, commitSha) {
    ...
    if (rocketComment?.body) {
        const updatedBody = rocketComment.body.replace(/Latest commit \*\*\[.*?\]\(.*?\)\*\*/i, `Latest commit **[${commitSha}](https://github.com/${owner}/${repo}/commit/${commitSha})**`);
